...
import os
from utils import get_dataloader_workers, pad, chamfer_distance, sinkhorn_emd, quat2mat
...
if __name__ == '__main__':
    ...
    checkpoints = torch.load(os.path.join(basedir, 'trainlog/exp4/model_40.pth'), map_location=device)
    model.load_state_dict(checkpoints["model"])

    for i, (pcd1, pcd2, f1, f2, q, t, pts1_gt, pts2_gt) in enumerate(dataloader):
        with torch.no_grad():
            ...
            # 1->2
            q12_pre, t12_pre, pts1_pre = model(pcd1, f1, pcd2, f2)
            varx = torch.var(pts1_pre[:, :, 0])
            vary = torch.var(pts1_pre[:, :, 1])
            varz = torch.var(pts1_pre[:, :, 2])
            q12_loss = nn.L1Loss()(q12_pre, q)
            t12_loss = torch.norm(t12_pre-t)
            # cd = chamfer_distance(pts_pre, pts_gt)
            emd12 = sinkhorn_emd(pts1_gt, pts1_pre)

            # 2->1
            q21_pre, t21_pre, pts2_pre = model(pcd2, f2, pcd1, f1)
            emd21 = sinkhorn_emd(pts2_gt, pts2_pre)
            # cycle loss
            R12 = quat2mat(q12_pre)
            T12 = torch.eye(4, device=device, dtype=torch.float32)
            T12[:3, :3] = R12
            T12[:3, 3] = t12_pre.squeeze(0)
            R21 = quat2mat(q21_pre)
        ...
        print(q12_loss, t12_loss, emd12, emd21, cyc_loss)
