...
from utils import get_dataloader_workers, sinkhorn_emd, loss_dispersion_ratio
...
if __name__ == '__main__':
    ...
    model = SonarNet().to(device) #SonarPCDNet().to(device)
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.9, patience=2, threshold=0.1, verbose=True)
    mseloss = nn.MSELoss()
    ...
    for epoch in range(Epochs):
        ...
        for i, (pcd1, pcd2, f1, f2, R, t, pts1_gt, pts2_gt) in tqdm(enumerate(train_dataloader), total=train_size):
            ...
            # pts1_pre, pts2_pre, phi1_pre, phi2_pre = model(pcd1, f1, pcd2, f2, pts1_gt.shape[1], f1[:, ::10, :2], f2[:, ::10, :2])
            ...
            # emd21 = sinkhorn_emd(pts2_gt, pts2_pre, eps=1e-3)
            # zloss1 = loss_dispersion_ratio(phi1_pre)
            # zloss2 = loss_dispersion_ratio(phi2_pre)

            # lamz = 1e-2 if 0.5*(zloss1+zloss2) > 0.1 else 0
            loss = pcd1_loss+pcd2_loss+1e3*(phi1_loss+phi2_loss) #1e2*R_loss+t_loss+pcd1_loss+pcd2_loss+1e2*(phi1_loss+phi2_loss)

            # rloss_train += R_loss
            # tloss_train += t_loss
            # emd_train += 0.5*(emd12+emd21)
            # x_train += x_loss
            # y_train += y_loss
            # z_train += 0.5*(zloss1+zloss2)
            # entropy_train += entropy_loss
            pcd_train += 0.5*(pcd1_loss+pcd2_loss)
            phi_train += 0.5*(phi1_loss+phi2_loss)
        ...
        scheduler.step(0.5*(phi1_loss+phi2_loss))
        ...
        phi_train = phi_train/train_size
        ...
        summary_writer.add_scalar('train/phi', phi_train, epoch)
        ...
        for j, (pcd1, pcd2, f1, f2, R, t, pts1_gt, pts2_gt) in tqdm(enumerate(test_dataloader), total=test_size):
            with torch.no_grad():
                ...
                # emd12 = sinkhorn_emd(pts1_gt, pts1_pre, eps=1e-3)
                # emd21 = sinkhorn_emd(pts2_gt, pts2_pre, eps=1e-3)
                # zloss1 = loss_dispersion_ratio(pts1_pre[..., 2])
                # zloss2 = loss_dispersion_ratio(pts2_pre[..., 2])
                rloss_test += R_loss
                tloss_test += t_loss
                pcd_test += 0.5*(pcd1_loss+pcd2_loss)
                phi_test += 0.5*(phi1_loss+phi2_loss)
                # emd_test += 0.5*(emd12+emd21)
                # x_train += x_loss
                # y_train += y_loss
                # z_test += 0.5*(zloss1+zloss2)
        ...
        summary_writer.add_scalar('test/phi', phi_test, epoch)
        ...
        torch.save(checkpoints, f'/media/kemove/043E0D933E0D7F44/Sonar_Diffusion_SLAM/trainlog/exp1/model_{epoch}.pth')